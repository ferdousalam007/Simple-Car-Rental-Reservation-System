...
const returnTheCarIntoDB = async (req:any) => {
    ...
    if(!carUpdate){
        throw new AppError(httpStatus.BAD_REQUEST, 'failed to update car startus'); 
    }
    const bookingUpdate= await Booking.findByIdAndUpdate(
        bookingId,
        { endTime: endTime, totalCost: totalCost },
        { new: true, session }
    ).populate("car").populate({ path: "user", select: { name: 1, email: 1, address: 1, role: 1, phone: 1 } })
    if(!bookingUpdate){
